                    ...
                    <div className='flex items-center justify-start'>
                        ...
                        <ul className='flex items-center gap-x-10'>
                            {navigationItem.map((navItem, index) => (
                                <Link key={index} to={navItem.href} ><li className='text-primary-10 lg:text-base md:text-xs'>{navItem.title}</li></Link>
                            ))}
                        </ul>
                    </div>
                </nav>
                <div className='flex items-center justify-center'>
                    <ul className='flex items-center justify-start gap-x-12 ' >
                        {navigationIcons.map((navItem, index) => (
                            <i key={index} className={`${navItem.iconClassName} text-primary-10 text-2xl`} > </i>
                        ))}
                    </ul>
                </div>
            </header>
                ...
                <section className='text-primary-10'>
                    ...
                    <div className='flex justify-center '>
                            ...
                            <div className='text-sm gap-y-3 flex-col flex pt-8'>
                                ...
                                <ul>
                                    {contactInfo.map((navItem, index) => (
                                        <li className='flex items-center justify-start gap-x-3 pb-2 ' key={index}>
                                            <i key={index} className={`${navItem.icon} text-primary-10 text-lg`} />
                        ...
                        <div className='flex flex-col gap-y-5 items-center justify-start'>
                            <img className='w-[122px] h-[122px]' src="/assetes/image/footer-images/2.png" alt="" />
